...
*/
 ...
 // if the local and code is zero, we're not completely off screen.
	   if (local_and == 0)
	   {
		...
		{
			visible[triangle] = hc_DecideFrontface(context, &(context->VertexBuffer[triangle]), &(context->VertexBuffer[triangle+1]), &(context->VertexBuffer[triangle+2]));
...
static INLINE void v_MtexToScreen_Smooth(GLcontext context, int i, int bufpointer)
{
	...
	mtex_pbuffer[bufpointer].color = CPACK(context->VertexBuffer[i].v.color.r, context->VertexBuffer[i].v.color.g, context->VertexBuffer[i].v.color.b, context->VertexBuffer[i].v.color.a);
}
...
//guardband-test:
   ...
   if(guard_band == 0)
   {
	...
	{
		if( hc_DecideFrontface(context, &context->VertexBuffer[0], &context->VertexBuffer[poly.verts[1]], &context->VertexBuffer[poly.verts[1] + 1]) == GL_FALSE)
		return;
	}
   ...
   else
   {
	...
	do
	{
		...
		{
		   if(hc_DecideFrontface(context, &context->VertexBuffer[0], &context->VertexBuffer[i], &context->VertexBuffer[i+1]) == GL_TRUE)
		   {
			nocull = 1;
		   }
		}
...
surgeon: added a version (d_DrawTrianglesVA) that uses W3D_DrawElements/W3D_DrawArray for unclipped chains of triangles. (DrawFn) Pointer is set in GLEnd, based on the number of triangles in the pipeline.
*/
...
void d_DrawTrianglesVA(GLcontext context)
{
	...
	for (i=0; i<context->VertexBufferPointer; i+=3)
	{
  	   ...
  	   else
	   {
		...
		if(context->CullFace_State == GL_TRUE && !(or_code & MGL_CLIP_NEGW))
		{
			if (hc_DecideFrontface(context, &(context->VertexBuffer[i+0]), &(context->VertexBuffer[i+1]), &(context->VertexBuffer[i+2])) == GL_FALSE)
			continue;
		}
	...
	else
	{
		...
		Set_W3D_TexCoordPointer(context->w3dContext, (void*)&(context->VertexBuffer->v.u), sizeof(MGLVertex), 0, 4, -4, W3D_TEXCOORD_NORMALIZED);
	}
	...
	if(context->ShadeModel == GL_SMOOTH)
	{
		...
		Set_W3D_ColorPointer(context->w3dContext, (void*)&(context->VertexBuffer->v.color.r), sizeof(MGLVertex), W3D_COLOR_FLOAT, W3D_CMODE_RGBA, 0);
	}
