...
void GLViewport(GLcontext context, GLint x, GLint y, GLsizei w, GLsizei h)
{
    ...
    context->ay = (float)(context->w3dWindow->Height - context->w3dWindow->BorderTop - context->w3dWindow->BorderBottom) - (float)y - (float)h * 0.5;
...
void GLEnd(GLcontext context)
{
    ...
    if (context->AlphaFellBack && (context->SrcAlpha == GL_ONE || context->DstAlpha == GL_ONE) && context->Blend_State == GL_TRUE)
    {
