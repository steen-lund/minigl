...
static GLboolean vid_OpenDisplay(GLcontext context, int pw, int ph, ULONG id)
{
    ...
    */
    ...
    {
    case W3D_NOGFXMEM:
        printf("No ZBuffer: Memory shortage\n");
        break;
    case W3D_NOZBUFFER:
        printf("No ZBuffer: Operation not supported\n");
        break;
    case W3D_NOTVISIBLE:
        printf("No ZBuffer: Screen is not visible\n");
        break;
    }
...
void GLClearColor(GLcontext context, GLclampf red, GLclampf green, GLclampf blue, GLclampf alpha)
{
    ...
    context->ClearColor = ((GLubyte)(alpha) << 24) + ((GLubyte)(red) << 16) + ((GLubyte)(green) << 8) + ((GLubyte)(blue));
}
...
GLboolean MGLLockDisplay(GLcontext context)
{
    ...
    else
    {
        ...
        {
        case W3D_NOTVISIBLE:
            printf("[MGLLockDisplay] Bitmap not visible\n");
            break;
        default:
            printf("[MGLLockDisplay] Error %d while locking\n", error);
            break;
        }
...
void *MGLCreateContextFromID(GLint id, GLint *width, GLint *height)
{
    ...
    if (id == MGL_SM_WINDOWMODE)
        newWindowMode = GL_TRUE;
