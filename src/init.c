...
GLboolean MGLInit(void)
{
...
#endif
    {
        ...
        if (!IntuitionBase)
            printf("- intuition.library (How are you doing this ?)\n");
        if (!GfxBase)
            printf("- graphics.library (Strange!)\n");

#ifdef __PPC__
        if (!Warp3DPPCBase)
            printf("- Warp3DPPC.library\n");
#else
        if (!Warp3DBase)
            printf("- Warp3D.library\n");
#endif
        if (!CyberGfxBase)
            printf("- cybergraphics.library\n");
